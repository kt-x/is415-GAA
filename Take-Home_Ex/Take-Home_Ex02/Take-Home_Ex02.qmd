...
Before we move into into EDA and thematic mapping, we need to calculate the cumulative vaccination cases rate (i.e. cases per 10000 population).

### CUMULATIVE CONFIRMED VACCINATION RATE

The cumulative vaccination rate can be calculated by taking the vaccinated (based on the name of the sub-district and the date), then dividing by the total population represented in our dataframe represents 10,000 denominations (in other words, this is number of cases for every 10,000 people). This can easily be achieved with pivot_wider().
...
vaccination_rate <- vaccine_jkt %>%
  ...
  dplyr::summarise(`vaccination_rate` = ((sum(Vaccinated)/(Total_Population))*10000)) %>%
...
Reference to In-class Exercise 7
...
```{r}
# vac_jul2021_st <- spacetime(vaccine_jkt, jkt,
#                       .loc_col = "Sub-District",
#                       .time_col = "2021-07-01")
# is_spacetime_cube(vac_jul2021_st)
```
